...
import type { User, Project, Organization } from 'umi';

import { useRequest } from '@/hooks';
import api from '@/api';

import styles from './InviteProject.less';
...
const InviteProject: React.FC<InviteProps> = ({
  ...
  const [checkedList, setCheckedList] = React.useState<number[]>(defaultValue.checkedList);
  const [checkAll, setCheckAll] = React.useState<boolean>(defaultValue.checkAll);
  const [indeterminate, setIndeterminate] = React.useState<boolean>(defaultValue.indeterminate);
        ...
        {users?.length ? (
          <div className={styles.content}>
            <div className={styles.projectItem}>
              ...
              <Checkbox checked={checkAll} onChange={handleCheckAll} indeterminate={indeterminate}>
                全选
              </Checkbox>
            </div>
            <Checkbox.Group
